...
export const NoticeModal: FC<NoticeModalProps> = ({ id, postSuccess, setNoticeId }) => {
    ...
    const searchDetail = (id: number) => {
        axios.post('/management/noticeFileDetail.do', { noticeId: id }).then((res: AxiosResponse<INoticeDetailResponse>) => {
    ...
    return (
        <NoticeModalStyled>
            <div className="container">
                <form ref={formRef}>
                    <label>
                        제목 :<input type="text" defaultValue={detail?.title} name="fileTitle"></input>
                    </label>
                    <label>
                        내용 : <input type="text" defaultValue={detail?.content} name="fileContent"></input>
                    </label>
                    ...
                    <div className={'button-container'}>
                        <button type="button" onClick={id ? updateNoticeFile : saveNoticeFile}>
                            {id ? '수정' : '저장'}
                        </button>
                        {!!id && (
